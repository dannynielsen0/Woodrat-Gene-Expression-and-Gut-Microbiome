...
y1 <- tax_glom(physeq1, taxrank = 'Family') # agglomerate taxa at desired level
...
y4$Family[y4$Abundance < 0.05] <- "Families < 5% abund." #rename genera with < .25% abundance
...
y4$Family <- factor(y4$Family, levels=rev(unique(y4$Family)))
...
mycolors <- colorRampPalette(brewer.pal(8, "Paired"))(length(unique(y4$Family)))


#plot
p <- ggplot(data=y4, aes(x=col2, y=Abundance, fill=Family, Family =Family)) +
...
#family
ggsave(plot=p, "../Lab-diet-trial-16S-analysis/figures/Family_RRA.jpg", width = 14, height =8 , device='jpg', dpi=500)
########
...
clostridium <- subset_taxa(physeq1, Family=="Erysipelotrichaceae")
